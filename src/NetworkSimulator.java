...
public class NetworkSimulator {
    ...
    public static Operation sendThroughNetwork(Operation operation) {
        ...
        if (errorChance < 30) { // 30% chance d'erreur
            ...
            if (field == 0) operation = InverserBit(operation, "operation");
            else if (field == 1) operation = InverserBit(operation, "operand1");
            else operation = InverserBit(operation, "operand2");
        }
    ...
    private static Operation InverserBit(Operation operation, String field) {
        ...
        //Ici, data.length() est la longueur de la chaîne binaire (par exemple "1010110011" a 10 caractères).
        //On fait data.length() - 2 car on ne veut PAS toucher aux 2 derniers bits (ils contiennent le checksum).


        char[] bits = data.toCharArray();
        //On utilise .toCharArray() pour transformer la chaîne en un tableau de caractères (char[]).
        //Pourquoi ?
        //Parce qu'on ne peut pas modifier directement un String en Java (il est immuable).
